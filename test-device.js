const { Host, VodRequestType } = require('./dist/cjs/index.js');
...
async function testDevice() {
  ...
  try {
    ...
    try {
      ...
      console.log(`  MAC Address: Not available (${err.message || String(err)})`);
    }
    ...
    try {
      ...
      console.log(`  Port information: Error retrieving (${err.message || String(err)})`);
    }
    ...
    try {
      ...
      console.log(`⚠ Error getting states: ${err.message || String(err)}`);
    }
    ...
    try {
      ...
      console.log(`  Baichuan info: ${err.message || String(err)}`);
    }
    ...
    try {
            ...
            if (last5Clips.length === 0) {
              ...
              for (let i = 0; i < last5Clips.length; i++) {
                ...
                try {
                  ...
                  console.log(`     Playback URL: ${url.substring(0, 80)}...`);
                } catch (urlErr) {
                  console.log(`     Playback URL: Error getting URL (${urlErr.message || String(urlErr)})`);
                }
              }
            }
          } catch (err) {
            console.log(`  Error getting clips: ${err.message || String(err)}`);
          }
        }
      } else {
      ...
      console.log(`⚠ Error getting video clips: ${err.message || String(err)}`);
    }
